...
====================================================`;
  ...
  return (
    <div className="min-h-screen bg-[#F5F1E8] text-[#2C3E50] flex flex-col items-center justify-center p-4 py-8">
      {/* Top Header */}
      <div className="w-full max-w-2xl mb-6 text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#1E3A5F] text-[#F5F1E8] rounded-full text-xs font-mono">
          <Sparkles size={14} className="text-[#4A9B9D]" />
          <span>RITUAL DE ONBOARDING · PASO {step} DE 7</span>
        </div>
        <h1 className="text-3xl font-serif font-bold text-[#1E3A5F]">
          Bienvenido al Manantial de MARU OS
        </h1>
        ...
        {/* Step Progress Bar */}
        <div className="w-full bg-[#E3DCCB] h-2 rounded-full overflow-hidden mt-4">
          <div
            className="bg-[#4A9B9D] h-full transition-all duration-500"
      ...
      {/* Main Card Container */}
      <div className="w-full max-w-2xl bg-white border border-[#E3DCCB] rounded-2xl shadow-xl p-6 sm:p-8 space-y-6">
        {/* PASO 1: CREAR CUENTA */}
        {step === 1 && (
          <div className="space-y-5">
            <div className="border-b border-[#E3DCCB] pb-4">
              <h2 className="text-xl font-serif font-bold text-[#1E3A5F] flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-[#F5F1E8]/50 border border-[#E3DCCB] rounded-xl text-sm text-[#2C3E50] focus:ring-2 focus:ring-[#4A9B9D] outline-none"
                  required
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Repetir Contraseña *</label>
                <input
                  type="password"
              ...
              <label className="flex items-start gap-3 p-3 bg-[#F5F1E8] rounded-xl border border-[#E3DCCB] cursor-pointer">
                <input
                  type="checkbox"
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 border border-[#E3DCCB] text-[#2C3E50] text-xs font-medium rounded-xl hover:bg-[#F5F1E8] transition-colors"
              >
                <Copy size={16} />
        ...
        {step === 3 && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">¿Cómo quieres que te llame?</label>
                <input
                  type="text"
                  value={profile.name}
                  onChange={(e) => setProfile({ ...profile, name: e.target.value })}
                  className="w-full px-4 py-2.5 bg-[#F5F1E8]/50 border border-[#E3DCCB] rounded-xl text-sm"
                />
              </div>

              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Edad (años)</label>
                <input
                  type="number"
                  value={profile.age}
              ...
              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Estatura (cm)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Peso (kg)</label>
                <input
                  type="number"
            ...
            <div>
              <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Sector de Trabajo</label>
              <select
            ...
            <div>
              <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Tono Preferido de Comunicación</label>
        ...
        {step === 4 && (
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-mono uppercase text-[#2C3E50]">Alergias Conocidas</label>
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                        isSelected
                          ...
                          : 'bg-[#F5F1E8] border-[#E3DCCB] text-[#2C3E50] hover:bg-[#E3DCCB]'
                      }`}
                    >
                      {isSelected ? '✓ ' : '+ '}{allergy}
                    </button>
              ...
              <div className="flex gap-2 pt-1">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 bg-[#F5F1E8]/50 border border-[#E3DCCB] rounded-xl text-xs"
                />
                <button
                  type="button"
            ...
            <div className="space-y-2 pt-2 border-t border-[#E3DCCB]">
              <label className="block text-xs font-mono uppercase text-[#2C3E50]">Medicamentos Actuales</label>
              <div className="space-y-2">
                {health.currentMedications.map((m) => (
                  <div key={m.id} className="flex items-center justify-between p-2.5 bg-[#F5F1E8] rounded-xl text-xs border border-[#E3DCCB]">
                    <div>
                      <span className="font-bold text-[#1E3A5F]">💊 {m.name}</span>
                      <span className="text-[#6B7F8C] ml-2">({m.dose} - {m.frequency})</span>
                    </div>
                    <button
                      type="button"
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                <input
                  ...
                  className="px-3 py-1.5 bg-[#F5F1E8]/50 border border-[#E3DCCB] rounded-xl text-xs"
                />
                <input
                  type="text"
                  value={medDose}
        ...
        {step === 5 && (
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">País</label>
                <input
                  type="text"
                  value={location.country}
                  disabled
                  className="w-full px-4 py-2.5 bg-[#E3DCCB]/40 border border-[#E3DCCB] rounded-xl text-sm font-semibold"
                />
              </div>

              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Ciudad</label>
                <select
                  value={location.city}
              ...
              <div>
                <label className="block text-xs font-mono uppercase text-[#2C3E50] mb-1">Distrito</label>
                <input
                  type="text"
        ...
        {step === 6 && (
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              {habits.map((h) => (
                <div key={h.id} className="flex items-center justify-between p-3 bg-[#F5F1E8] border border-[#E3DCCB] rounded-xl text-xs">
                  <div className="flex items-center gap-3">
                    <span className="font-mono bg-[#1E3A5F] text-white px-2 py-0.5 rounded text-[10px]">{h.time}</span>
                    <span className="font-medium text-[#2C3E50]">{h.title}</span>
                  </div>
                  <span className="text-[#6B7F8C] text-[11px]">{h.frequency}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <input
                ...
                className="flex-1 px-3 py-2 bg-[#F5F1E8]/50 border border-[#E3DCCB] rounded-xl text-xs"
              />
              <button
                type="button"
                onClick={() => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-xl border cursor-pointer transition-all ${
                      isChecked
                        ? 'border-[#4A9B9D] bg-[#4A9B9D]/10 text-[#1E3A5F]'
                        : 'border-[#E3DCCB] bg-[#F5F1E8]/40'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="font-bold text-sm flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: agent.colorAccent }} />
                        {agent.name} <span className="text-xs font-normal text-[#6B7F8C]">({agent.specialty.split('&')[0]})</span>
                      </div>
                      <input type="checkbox" checked={isChecked} readOnly className="accent-[#4A9B9D]" />
                    </div>
                    <p className="text-[11px] text-[#6B7F8C] mt-1 italic font-serif">"{agent.catchphrase}"</p>
                  </div>
                );
              })}
            </div>

            <label className="flex items-center gap-3 p-3 bg-[#F5F1E8] border border-[#E3DCCB] rounded-xl cursor-pointer">
              <input
                type="checkbox"
                checked={autoRouting}
        ...
        {/* Navigation Buttons */}
        <div className="flex items-center justify-between pt-4 border-t border-[#E3DCCB]">
          {step > 1 ? (
            <button
              type="button"
              onClick={() => setStep(step - 1)}
              className="flex items-center gap-2 px-4 py-2.5 border border-[#E3DCCB] rounded-xl text-xs font-medium text-[#2C3E50] hover:bg-[#F5F1E8]"
            >
              <ArrowLeft size={16} />
              Anterior
            </button>
